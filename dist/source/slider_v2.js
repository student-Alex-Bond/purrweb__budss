window.addEventListener('DOMContentLoaded', () => {
    ...
    const renderDots = () => {
        ...
        const arrayButtons = [];
        for (let i = 0; i <= arrayFromSourceImages.length - 1; i += 1) {
    ...
    const animateSlide = (direction) => {
        const currentSlides = document.querySelectorAll('.slides__slide');
        const start = Date.now();

        const timer = setInterval(() => {
            const timePassed = Date.now() - start;
            const slideWidth = currentSlides[firstImage].offsetWidth;
            ...
            if (direction === 'next') {
                currentSlides[firstImage].style.left = `${Math.ceil(-timePassed / divider)}px`;
                currentSlides[secondImage].style.left = `${Math.ceil(slideWidth - (timePassed / divider))}px`;
            } else {
                currentSlides[secondImage].style.left = `${Math.ceil(timePassed / divider)}px`;
                currentSlides[firstImage].style.left = `${Math.ceil(-slideWidth + (timePassed / divider))}px`;
            }
        }, 17);
    };
    ...
    const showClickedSlide = (event) => {
        ...
        if (id) {
            ...
            currentIndex = Number(id); // <=bad ошибкa(отсутсвие строгой типизации)
            toggleActiveDot(currentIndex);
            animateSlide('next');
        }
    };
